...
- [ ] Update config example with new sections (`models`, `relay_providers`, `stripe`).
...
- Sales tax/VAT is handled by Stripe where possible.
...
3. Should Stripe handle sales tax automatically, or do we need a separate tax provider?
   - **Stripe Tax is fine for US launch.** Since LibreLoom is US-based, Stripe's automatic tax handling is the right starting point.
