...
import { LayoutDashboard, CreditCard, ChartBar, Package, LogOut, Sun, Moon } from "lucide-react";
    ...
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <aside className="fixed inset-y-0 left-0 z-20 flex w-64 flex-col border-r border-border bg-card">
        <div className="flex h-16 items-center gap-2 px-6">
          ...
          <span className="font-mono text-lg font-bold">Connect</span>
        </div>
        <Separator />
        ...
        <div className="p-3 space-y-2">
          {device && (
            <div className="px-4 py-2">
              <p className="font-mono text-xs text-muted-foreground">Current Plan</p>
              <p className="font-mono text-sm font-bold">{device.plan_name}</p>
            </div>
          )}
