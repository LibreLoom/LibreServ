    ...
    <Layout>
      ...
      <div className="grid gap-4 md:grid-cols-2 mb-6">
        <Card>
          ...
          <CardContent>
            <p className="font-mono text-2xl">${(usage?.total_cost_usd || 0).toFixed(2)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-sm text-muted-foreground">Credits Used</CardTitle></CardHeader>
          <CardContent>
            <p className="font-mono text-2xl">${((usage?.credits_used || 0) / 100).toFixed(2)}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="space-y-3">
          {entries.map(([key, svc]) => (
            <Card key={key} className="flex items-center justify-between">
              <div>
                <p className="font-mono">{serviceLabels[key] || key}</p>
                <p className="text-sm text-muted-foreground">Usage: {svc.value.toFixed(2)}</p>
              </div>
              <div className="text-right">
                <p className="font-mono">${svc.cost_usd.toFixed(2)}</p>
