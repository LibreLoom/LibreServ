...
Staff admin: first account via `/admin/seed` (loopback, or `auth.admin_seed_token` + `X-Seed-Token`), then `/admin/login`. Bulk factory lists download as a file named **`TOKENS`** (6-digit uppercase hex, one per line).
