...
| `sol/server/backend/OS/dist/` | Script (frontend) | Never |
| `sol/server/backend/bin/` | Not by script | Never |
...
- `cd sol/server/backend && go mod download`
- `cd sol/server/frontend && npm install`
