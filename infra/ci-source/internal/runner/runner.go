...
func (r *Runner) createContainer(ctx context.Context, t *tests.Test, fuzzDuration time.Duration) (string, error) {
	...
	if strings.HasPrefix(t.ID, "frontend-") {
		...
		binds = append(binds, nodeModulesCache+":/repo/sol/server/frontend/node_modules:z")
...
func findRepoRoot() (string, error) {
	...
	dir := cwd
	for {
		if _, err := os.Stat(filepath.Join(dir, "sol", "server")); err == nil {
			if _, err := os.Stat(filepath.Join(dir, "sol", "connect")); err == nil {
				return dir, nil
			}
		}
		...
		parent := filepath.Dir(dir)
		if parent == dir {
			return "", fmt.Errorf("could not find repo root (looked for sol/server/ and sol/connect/ dirs)")
		}
		dir = parent
	}
}
