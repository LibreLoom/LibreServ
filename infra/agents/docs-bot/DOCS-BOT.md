...
No pull-request reviews. No webhook. Not the deleted infra/agents/nightly harness or docs-updater.
...
`/data/LibreServ/infra/agents/docs-bot/cook.sh`. Secrets stay in the env_file only.
...
- or `docker exec docs-bot /data/LibreServ/infra/agents/docs-bot/cook.sh`
...
`common/forgejo.sh` (shared) does REST over curl. Keep it that way.
