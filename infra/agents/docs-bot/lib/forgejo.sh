#!/usr/bin/env bash
# Tiny Forgejo REST helpers for docs-bot. Never print the token.
# Authenticate as docs-bot. Never the Actions token.
set -euo pipefail
FORGEJO_URL="${FORGEJO_URL:-https://gt.plainskill.net}"
if [[ -z "${DOCS_BOT_TOKEN:-}" ]]; then
  echo "forgejo.sh: DOCS_BOT_TOKEN is required (will not fall back to FORGEJO_TOKEN or ATLAS_BOT_TOKEN)" >&2
  exit 2
fi
TOKEN="${DOCS_BOT_TOKEN}"
