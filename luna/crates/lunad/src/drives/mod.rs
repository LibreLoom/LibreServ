...
impl DriveManager {
    pub fn new(mounter: Arc<dyn Mounter>, data_dir: &Path) -> Self {
        Self::new_with_probe(
            mounter,
            data_dir,
            Arc::new(crate::drives::fsprobe::CommandFsProbe),
        )
    }
    ...
    pub fn adopt(
        ...
        if let Err(e) = crate::drives::drive_db::create(&mount_point, &marker, &prefix) {
            if is_erofs(&e) {
                ...
                match self.remount_adopt_rw(device, &choice, &id, &mount_point, mounted_by_luna) {
                    Ok((new_mp, new_by_luna)) => {
                        ...
                        mounted_by_luna = new_by_luna;
                        if let Err(e2) =
                            crate::drives::drive_db::create(&mount_point, &marker, &prefix)
                        {
                            self.cleanup_adopt_mount(&mount_point, mounted_by_luna);
                            if is_erofs(&e2) {
...
mod tests {
    ...
    fn remove_on_os_mount_only_deletes_marker() {
        ...
        let row = mgr
            ...
            .unwrap();
        assert!(crate::drives::drive_db::find_db_file(&existing).is_some());

        mgr.remove(&conn, &row.id).unwrap();
        assert!(db::get_drive(&conn, &row.id).unwrap().is_none());
        assert!(crate::drives::drive_db::find_db_file(&existing).is_none());
        assert_eq!(
    ...
    fn remove_readonly_unregisters_when_marker_undeletable() {
        ...
        let marker = crate::drives::drive_db::find_db_file(&existing).unwrap();
    ...
    fn remove_unplugged_forgets_db_row_without_marker() {
        ...
        let row = mgr
            .adopt(&conn, &detected("sdz", None), "Backup Drive", false)
            .unwrap();
        let marker = crate::drives::drive_db::find_db_file(Path::new(&row.mount_point)).unwrap();
        assert!(marker.is_file());
        mgr.reconcile(&conn, &[]).unwrap();
        assert_eq!(
            db::get_drive(&conn, &row.id).unwrap().unwrap().state,
    ...
    fn iso_probe(disk: &str) -> Arc<MockFsProbe> {
        ...
        probe.disks.lock().unwrap().insert(
            disk.into(),
            vec![crate::drives::fsprobe::FsInfo {
                name: disk.into(),
