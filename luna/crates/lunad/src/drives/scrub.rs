...
pub fn hash_drive(
    ...
    let _ = crate::drives::drive_db::open_migrating(root, central, drive_id)?;
...
pub fn hash_drive_unlocked(
...
) -> anyhow::Result<ScrubReport> {
    {
        ...
        let conn = crate::drives::drive_db::open(root)?;
        crate::drives::drive_db::migrate_from_central(&central, drive_id, &conn)?;
    }
    hash_drive_walk(drive_id, root)
}
...
mod tests {
    ...
    fn hash_drive_keeps_an_unchanged_baseline() {
        ...
        std::fs::create_dir_all(&root).unwrap();
        let marker = luna_core::marker::Marker::new("d1", "D");
        let _ = crate::drives::drive_db::create(
            &root,
            &marker,
            &luna_core::marker::pick_prefix(&root).unwrap(),
        )
