...
/**
 * A real xlsx from parsed rows (csv/tsv conversion). Strings go in as
 * inlineStr cells so no sharedStrings bookkeeping is needed; plain numbers
 * become numeric cells so totals still compute.
 *
 * @param {string[][]} rows
 * @returns {Blob}
 */
...
/** @param {number} index 0-based column → spreadsheet letters (0→A, 26→AA). */
...
/**
 * Values like "007" or "1e5" must stay text; only plain decimal strings
 * become numeric cells so a product code never silently rewrites itself.
 * @param {string} value
 */
