...
#   LUNA_DEV_PASS / LUNA_DESKTOP_DEV_PASS   (default: hunter22hunter1)
...
PASS="${LUNA_DEV_PASS:-${LUNA_DESKTOP_DEV_PASS:-hunter22hunter1}}"
...
curl -fsS -c "$COOKIE_JAR" -b "$COOKIE_JAR" -D "$HDR" \
  ...
  || {
    ...
    echo "LUNA_DEV_USER / LUNA_DEV_PASS (or LUNA_DESKTOP_DEV_USER / LUNA_DESKTOP_DEV_PASS)." >&2
    exit 1
  }
