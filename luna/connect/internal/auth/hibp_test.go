...
import (
	"crypto/sha1" //nolint:gosec // SHA-1 required by HIBP; only prefix transmitted
	"encoding/hex"
	"net/http"
	"net/http/httptest"
	"strings"
...
func TestRejectBreachedPasswordMessage(t *testing.T) {
	sum := sha1.Sum([]byte("password123")) //nolint:gosec
	full := strings.ToUpper(hex.EncodeToString(sum[:]))
	suffix := full[5:]
	...
	msg := RejectBreachedPassword("password123")
	if msg != BreachedPasswordMessage {
		t.Fatalf("got %q", msg)
	}
}
