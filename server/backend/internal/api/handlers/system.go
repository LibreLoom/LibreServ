...
func (h *SystemHandler) SetAuditLogger(fn func(ctx context.Context, action, targetID, targetName, status, message string, metadata map[string]interface{})) {
	h.auditLog = fn
}
...
func (h *SystemHandler) ApplyUpdate(w http.ResponseWriter, r *http.Request) {
	if err := h.checker.ApplyUpdate(r.Context(), Version); err != nil {
		if h.auditLog != nil {
			h.auditLog(r.Context(), "system.update", "", "libreserv", "failure", err.Error(), nil)
		}
	...
	if h.auditLog != nil {
		h.auditLog(r.Context(), "system.update", "", "libreserv", "success", "System update applied", nil)
	}
