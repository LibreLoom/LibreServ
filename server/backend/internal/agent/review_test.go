...
func TestReviewModelNilSafety(t *testing.T) {
	...
	var rm *ReviewModel
	result, err := rm.Review(context.Background(), "test request", "bash", nil, "no context", false)
	if err != nil {
...
func TestReviewModelNilSafetyAutoMode(t *testing.T) {
	...
	var rm *ReviewModel
	result, err := rm.Review(context.Background(), "test request", "bash", nil, "no context", true)
	if err != nil {
		t.Fatalf("nil review model should not error: %v", err)
	}
