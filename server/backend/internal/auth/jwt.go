...
func (j *JWTManager) GenerateTokenPair(userID, username, role string) (*TokenPair, error) {
	accessToken, expiresAt, err := j.generateToken(userID, username, role, "access", j.accessTokenExpiry)
	if err != nil {
		return nil, err
	}

	refreshToken, _, err := j.generateToken(userID, username, role, "refresh", j.refreshTokenExpiry)
	if err != nil {
		return nil, err
	}
...
func (j *JWTManager) generateToken(userID, username, role, tokenType string, expiry time.Duration) (string, int64, error) {
