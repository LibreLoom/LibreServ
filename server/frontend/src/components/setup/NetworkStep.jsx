 ...
 *   - While already online by cable, Wi-Fi stays joinable through the
 *     "Also connect Wi-Fi (optional)" panel — it never blocks progress.
 */
...
export default function NetworkStep({ name, onContinue }) {
  ...
  const [showOptionalWifi, setShowOptionalWifi] = useState(false);
  ...
  const wifiConnected = Boolean(status?.connected);
  // Until the first status arrives, assume the device has a radio so the
  // picker renders optimistically instead of flashing the cable instruction.
  const wifiAvailable = status ? Boolean(status.available) : true;
  const online = ethernet || wifiConnected;

  // The picker shows while we're offline, or once the user opens the
  // optional "also connect Wi-Fi" panel while already online by cable.
  const showPicker = wifiAvailable && (!online || (showOptionalWifi && !wifiConnected));
  ...
  const handleConnect = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      // The response carries the fresh status — apply it so the board flips
      // immediately; the 3s poll keeps it honest afterwards. The step does
      // NOT advance on its own: the user confirms with Continue.
      if (aliveRef.current && data) setStatus(data);
      setPassword("");
      setSelected("");
    } catch {
  ...
  // The Wi-Fi picker (list + password + error). Rendered either while
  // offline, or inside the optional panel while already online by cable.
  const wifiPicker = (
    <div className="w-full max-w-sm space-y-2 text-left" data-slot="network-wifi-picker">
      <p className="font-mono text-sm text-primary mb-1">Pick your home Wi-Fi</p>
  ...
  // The connect button shared by both picker contexts.
  const connectButton = (
    <Button
      variant="primary"
      fullWidth
    ...
    <div className="flex flex-col items-center text-center py-2" data-slot="setup-network-step">
      ...
      <p className="text-primary/68 text-sm leading-relaxed max-w-md mb-8">
        {online
          ...
          : wifiAvailable
            ? `${name} needs a way to reach your internet. Plug a cable into the back, or pick your home Wi-Fi below — either one works.`
      ...
      <div className="w-full max-w-sm space-y-2 mb-8" data-slot="network-board">
        ...
        <BoardRow
          ...
          label="Wi-Fi"
          ok={wifiConnected}
          okText={status?.ssid ? `Connected to ${status.ssid}` : "Connected"}
          offText={wifiAvailable ? "Not connected" : "Not available"}
        />
      </div>

      {/* Offline: join Wi-Fi — or, when there's no radio, plug a cable. */}
      {!online && showPicker && (
        <div className="w-full flex flex-col items-center space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
          {wifiPicker}
          {connectButton}
        </div>
      )}
      ...
      {/* Online: one exit (Continue), plus the optional other path. */}
      {online && (
        <div className="w-full max-w-sm flex flex-col items-center space-y-5 text-left animate-in fade-in slide-in-from-bottom-2 duration-300" data-slot="network-online">
          ...
          <Button
            variant={showPicker ? "outline" : "primary"}
            fullWidth
            onClick={onContinue}
