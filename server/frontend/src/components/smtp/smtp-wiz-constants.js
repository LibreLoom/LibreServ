export const WIZ = {
  PROVIDER_PICK: "smtp_provider_pick",
  CREDENTIALS:   "smtp_credentials",
  TESTING:       "smtp_testing",
  CONNECTED:     "smtp_connected",
  SKIP_CONFIRM:  "skip_confirm",
};

export const SMTP_PRESETS = {
  proton: {
    id: "proton",
    label: "Proton",
    host: "smtp.protonmail.ch",
    port: 587,
    use_tls: true,
    skip_verify: false,
    username_label: "Email address",
    password_label: "SMTP token",
    help: "Generate an SMTP token at Settings → IMAP/SMTP → SMTP tokens. Use your custom domain email as the username.",
    docs_url: "https://proton.me/support/smtp-submission",
  },
  resend: {
    id: "resend",
    label: "Resend",
    host: "smtp.resend.com",
    port: 587,
    use_tls: true,
    skip_verify: false,
    username_label: "Username",
    password_label: "API key",
    help: 'Use "resend" as the username and your API key as the password.',
    docs_url: "https://resend.com/docs/send-with-smtp",
  },
  postmark: {
    id: "postmark",
    label: "Postmark",
    host: "smtp.postmarkapp.com",
    port: 587,
    use_tls: true,
    skip_verify: false,
    username_label: "Server API token",
    password_label: "Server API token",
    help: "Use the same Server API token for both username and password.",
    docs_url: "https://postmarkapp.com/developer/sending#smtp",
  },
  custom: {
    id: "custom",
    label: "Custom SMTP",
    host: "",
    port: 587,
    use_tls: true,
    skip_verify: false,
    username_label: "Username",
    password_label: "Password",
    help: "Enter your SMTP server details manually.",
    docs_url: "",
  },
};
