...
import { serializeShakeTrigger, shakeElement } from "../utils/shake";
...
export default function useShakeOnError(trigger, ref, options) {
  ...
  useEffect(() => {
    if (resubmitBeat === 0) return;

    const triggerForResubmit = activeTrigger || previousTrigger.current;
    if (!triggerForResubmit) return;
