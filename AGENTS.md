...
- **Atlas-bot** lives on Forgejo, not GitHub. Mention `@atlas-bot` or assign the issue/PR. Authz is live LibreLoom Owners team membership. Runtime is `agents/atlas-runner` (podman, host executor inside the container). Do not reuse `agents/nightly`. See `ATLAS-BOT.md`.
