...
build_and_install() {
    ...
    (cd "$REPO_ROOT/sol/connect/web/admin" && npm install --silent && npm run build)

    log_info "  Building customer UI..."
    (cd "$REPO_ROOT/sol/connect/web/customer" && npm install --silent && npm run build)
    ...
    (cd "$REPO_ROOT/sol/connect" && go build \
    ...
    cp -r "$REPO_ROOT/sol/connect/web/admin/dist" "$INSTALL_DIR/web/admin"
    cp -r "$REPO_ROOT/sol/connect/web/customer/dist" "$INSTALL_DIR/web/customer"
...
print_caddy_instructions() {
    ...
    echo "    cd $REPO_ROOT && ./sol/connect/deploy/deploy.sh"
    echo ""
    echo "  If using --tag:"
    echo "    ./sol/connect/deploy/deploy.sh --tag vX.Y.Z"
    echo ""
