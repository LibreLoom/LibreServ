...
#
# Usage:
#   ./sol/connect/deploy/deploy.sh                   # deploy latest connect-v* tag (default)
#   ./sol/connect/deploy/deploy.sh --tag connect-v1.2.3  # deploy a specific tag
#   ./sol/connect/deploy/deploy.sh --head            # deploy current HEAD (dev escape hatch)
#
...
build_binary() {
    ...
    cp -a "$REPO_ROOT/sol/connect/web/admin/dist/." "${INSTALL_DIR}/web/admin/"
    cp -a "$REPO_ROOT/sol/connect/web/customer/dist/." "${INSTALL_DIR}/web/customer/"
