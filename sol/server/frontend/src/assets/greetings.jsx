// Rotating friendly copy to make idle/loading states feel less sterile.
export const dashboard = [
  "Welcome back. The pigeon is managing operations and snacks, ",
  "Hi there. The mouse intern arrived early and looks determined, ",
  "Hello! The pigeon says today is ‘under control’, ",
  "Welcome! Snack inventory is stable and morale is high, ",
  "Hey there. The seagull was spotted but the pigeon remains calm, ",
  "Welcome back. The raccoon accountant is balancing the snack budget, ",
  "Hello! The pigeon adjusted its clipboard and nodded confidently, ",
  "Welcome! The mouse intern labeled today ‘promising’, ",
  "Hi there. The turtle from HR is observing quietly, ",
  "Welcome back. The pigeon believes this is a good moment to begin, ",

  "Hello! The pigeon reorganized things and feels accomplished, ",
  "Welcome! The snacks are exactly where they should be, ",
  "Hey there. The seagull asked questions. The pigeon declined to answer, ",
  "Welcome back. The raccoon accountant requested a recount, ",
  "Hi there. The mouse intern took notes and added doodles, ",
  "Hello! The pigeon says progress is happening ‘slowly but correctly’, ",
  "Welcome! The owl auditor has not appeared. Everyone is relieved, ",
  "Hey there. The turtle from HR approved today’s tone, ",
  "Welcome back. The pigeon inspected everything twice, ",
  "Hello! Snack morale has been officially acknowledged, ",

  "Welcome! The pigeon filed today under ‘manageable’, ",
  "Hi there. The mouse intern is practicing professionalism, ",
  "Hello! The raccoon accountant says the numbers look ‘friendly’, ",
  "Welcome back. The pigeon is pleased with the level of order, ",
  "Hey there. The seagull is nearby but pretending not to be, ",
  "Welcome! The snacks are resting comfortably, ",
  "Hello! The turtle from HR scheduled nothing urgently, ",
  "Welcome back. The pigeon says this is a safe place to start, ",
  "Hi there. The mouse intern believes in this moment, ",
  "Welcome! The clipboard has been consulted, ",

  "Hello! The pigeon is quietly proud of this setup, ",
  "Welcome back. The raccoon accountant made a note and smirked, ",
  "Hey there. The seagull was denied entry politely, ",
  "Welcome! Snack access remains fair and just, ",
  "Hi there. The turtle from HR nodded approvingly, ",
  "Welcome back. The pigeon says things are aligned, ",
  "Hello! The mouse intern filed something under ‘important’, ",
  "Welcome! The owl auditor remains a rumor, ",
  "Hey there. The pigeon adjusted its tiny tie, ",
  "Welcome back. Order has been gently applied, ",

  "Hello! The pigeon declared this a ‘working atmosphere’, ",
  "Welcome! The snacks are part of the plan, ",
  "Hi there. The raccoon accountant double-checked everything, ",
  "Welcome back. The seagull is waiting outside dramatically, ",
  "Hey there. The mouse intern asked a good question, ",
  "Welcome! The turtle from HR is very calm about everything, ",
  "Hello! The pigeon says clarity is present, ",
  "Welcome back. The clipboard is satisfied, ",
  "Hi there. Snack crumbs indicate recent success, ",
  "Welcome! This feels officially organized, ",

  "Hello! The pigeon believes momentum is building, ",
  "Welcome back. The raccoon accountant approved today’s math, ",
  "Hey there. The seagull attempted eye contact and failed, ",
  "Welcome! The mouse intern labeled this ‘Phase One’, ",
  "Hi there. The turtle from HR remains unbothered, ",
  "Welcome back. The pigeon says things are proceeding nicely, ",
  "Hello! Snack morale is holding steady, ",
  "Welcome! The owl auditor has not requested documentation, ",
  "Hey there. The pigeon marked today as ‘good’, ",
  "Welcome back. Everything is politely arranged, ",

  "Hello! The pigeon trusts this configuration, ",
  "Welcome! The raccoon accountant adjusted the ledger, ",
  "Hi there. The mouse intern is learning quickly, ",
  "Welcome back. The seagull was redirected successfully, ",
  "Hey there. The turtle from HR approved a flexible pace, ",
  "Welcome! Snacks are being treated with respect, ",
  "Hello! The pigeon checked twice and smiled once, ",
  "Welcome back. The clipboard feels important, ",
  "Hi there. This is the calm version of things, ",
  "Welcome! Order is present and comfortable, ",

  "Hello! The pigeon believes today is promising, ",
  "Welcome back. The raccoon accountant counted crumbs, ",
  "Hey there. The seagull is negotiating from afar, ",
  "Welcome! The mouse intern requested mentorship, ",
  "Hi there. The turtle from HR scheduled a check-in eventually, ",
  "Welcome back. The pigeon says everything is fine, ",
  "Hello! Snacks have been acknowledged officially, ",
  "Welcome! The owl auditor remains theoretical, ",
  "Hey there. The clipboard has been updated, ",
  "Welcome back. This is a good place to continue, ",

  "Hello! The pigeon declared this a low-drama moment, ",
  "Welcome! The raccoon accountant nodded knowingly, ",
  "Hi there. The mouse intern is feeling optimistic, ",
  "Welcome back. The seagull has been distracted elsewhere, ",
  "Hey there. The turtle from HR smiled faintly, ",
  "Welcome! Snacks are calm and cooperative, ",
  "Hello! The pigeon says things are aligned enough, ",
  "Welcome back. The clipboard agrees, ",
  "Hi there. Everything feels manageable here, ",
  "Welcome! The pigeon is ready when you are, ",

  "Hello! The pigeon adjusted the rules slightly, ",
  "Welcome back. The raccoon accountant approved the change, ",
  "Hey there. The mouse intern celebrated quietly, ",
  "Welcome! The seagull is temporarily uninterested, ",
  "Hi there. The turtle from HR is satisfied, ",
  "Welcome back. Snacks remain a unifying force, ",
  "Hello! The pigeon believes this is the right moment, ",
  "Welcome! Everything is set up thoughtfully, ",
  "Hey there. The clipboard has no objections, ",
  "Welcome back. The pigeon-verse is stable, ",
];

// 404 quips used by the NotFoundPage; keep tone playful but clear.
export const notfound = [
  "The pigeon checked the map. Then checked it again. This page is not on it.",
  "The mouse ran out of snacks halfway though building this page. The mouse intern is still waiting for their snacks.",
  "The page is missing. The pigeon filed the paperwork immediately.",
  "We asked the pigeon. The pigeon asked the mouse intern. The intern shrugged.",
  "The pigeon opened this door carefully. There was nothing behind it.",
  "According to the pigeon, this page was here earlier. Allegedly.",
  "The pigeon searched high and low. Mostly low. Still no page.",
  "The page is missing. The raccoon accountant says the numbers don’t add up.",
  "The pigeon suspects crumbs are involved. The seagull refuses to comment.",
  "This page took a wrong turn. The pigeon made a note.",
  "The pigeon left a sign here that says ‘Not this way’. Very official.",

  "The mouse intern swears this page existed. The pigeon remains unconvinced.",
  "The pigeon checked the snack drawer first. Force of habit.",
  "This page is absent. The turtle from HR is remaining calm about it.",
  "The pigeon believes this is a navigation issue, not a personal failure.",
  "The owl auditor looked once and said nothing. That was worse.",
  "The pigeon marked this page as ‘missing but polite’.",
  "We searched everywhere. The seagull searched *enthusiastically*. No page.",
  "The pigeon adjusted its tiny tie and apologized professionally.",
  "This page has gone rogue. The pigeon is drafting a policy.",
  "The raccoon accountant insists the page was never budgeted for.",

  "The pigeon retraced every step. The steps did not lead here.",
  "This page is currently unavailable due to being completely gone.",
  "The pigeon believes this page is ‘on a break’.",
  "We asked nicely. The pigeon asked firmly. The page still didn’t return.",
  "The mouse intern labeled this incident ‘educational’.",
  "The pigeon suspects the seagull knows something.",
  "This page is missing. Snack morale has been affected slightly.",
  "The pigeon checked behind the couch. Still nothing.",
  "The turtle from HR says this is ‘one of those things’.",
  "The pigeon filed this under ‘unexpected but manageable’.",

  "The page is not here. The pigeon would like you to know it tried.",
  "This link went on an adventure. The pigeon stayed behind.",
  "The pigeon believes the page took a scenic route and forgot to come back.",
  "We looked everywhere reasonable. The pigeon looked everywhere unreasonable.",
  "The pigeon insists this is not anyone’s fault.",
  "The owl auditor wrote ‘404’ and stared meaningfully.",
  "The pigeon left breadcrumbs. The page did not follow them.",
  "This page is missing. The pigeon recommends returning to safety.",
  "The raccoon accountant says losses are within acceptable limits.",
  "The pigeon says this happens sometimes and it’s okay.",

  "The page is gone. The pigeon is already reorganizing around it.",
  "This is an empty room. The pigeon checked twice.",
  "The pigeon is confident the correct page is nearby.",
  "We found the problem. The page isn’t here.",
  "The mouse intern learned a valuable lesson just now.",
  "The pigeon marked this location ‘non-page’.",
  "The seagull flew by laughing. The pigeon took notes.",
  "This page failed to report for duty.",
  "The pigeon assures you everything else is still in order.",
  "If this page existed, the pigeon would have organized it already.",
];

// Missing object quips used by ObjectNotFound; keep tone playful but specific.
export const objectnotfound = [
  "The pigeon checked the Clipboard. This entry is not there.",
  "The mouse intern swears they filed it. The mark is missing.",
  "The raccoon accountant says this entry was never budgeted for.",
  "We asked the Clipboard. The Clipboard offered a polite shrug.",
  "The pigeon opened the cabinet labeled 'this thing'. It was empty.",
  "The turtle from HR suggests this item might be on a long break.",
  "The pigeon followed the crumbs. They stopped abruptly.",
  "The owl auditor took one look and wrote 'missing'.",
  "The record took a detour. The pigeon took notes.",
  "We looked where it should be. The pigeon looked everywhere else.",
  "The mouse intern labeled this incident 'still unresolved'.",
  "The pigeon suspects a typo is hiding in plain sight.",
  "This entry is absent. The pigeon recommends a deep breath.",
  "The list is quiet. The item is quieter.",
  "The pigeon is confident it exists, just not here.",
  "We checked the usual places. Then the unusual ones.",
  "The seagull flew by laughing. The pigeon did not laugh.",
  "The pigeon marked this as 'missing but probably fine'.",
  "This record failed to report for duty.",
  "The pigeon recommends returning to safety and trying again.",

  "The mouse intern checked the index twice. Nothing.",
  "The pigeon tapped the folder. No response.",
  "The cabinet sighed. The record did not appear.",
  "The raccoon accountant says the trail ends here.",
  "We opened the drawer marked 'definitely here'. It was not.",
  "The pigeon placed a missing tag and a tiny sigh.",
  "The turtle from HR suggests a calm retry.",
  "The owl auditor flipped a page and frowned.",
  "We asked nicely. The record stayed missing.",
  "The pigeon made a note: check the spelling.",

  "The mouse intern looked behind the binder. Still empty.",
  "The record took the scenic route and never returned.",
  "The Clipboard remains confident but unhelpful.",
  "We looked high. We looked low. We found nothing.",
  "The pigeon checked twice and asked once. No record.",
  "The seagull took a lap and offered no clues.",
  "The raccoon accountant says the numbers disagree.",
  "The record is absent. The pigeon is polite about it.",
  "This item is missing from the roster.",
  "The pigeon checked the logbook. Blank.",

  "The mouse intern left a sticky note: 'still missing'.",
  "We searched politely. The record declined to appear.",
  "The turtle from HR nods: it happens.",
  "The owl auditor wrote 'not found' in tiny letters.",
  "The pigeon suspects an old link is involved.",
  "The file is gone. The folder is innocent.",
  "The record went to get snacks and did not return.",
  "The pigeon suggests going back to the list.",
  "We checked the index. The index checked us back.",
  "The cabinet is empty and proud of it.",

  "The mouse intern filed a report: 'missing'.",
  "The raccoon accountant says the entry never existed.",
  "The pigeon adjusted the label and looked again. Nothing.",
  "The record is missing. The room is quiet.",
  "We asked the ledger. The ledger refused to comment.",
  "The pigeon checked the obvious place. Then the obvious-er place.",
  "The turtle from HR recommends a deep breath.",
  "The owl auditor raised an eyebrow and moved on.",
  "The record did not check in today.",
  "The mouse intern promises they tried.",

  "We knocked on the folder. No answer.",
  "The pigeon placed a tiny cone: 'missing area'.",
  "The seagull claims innocence. The pigeon is skeptical.",
  "The record is not in the list at the moment.",
  "We looked in the archive. Not there either.",
  "The pigeon wrote 'try again' and circled it.",
  "The raccoon accountant says the entry is overdrawn.",
  "The record declined to be found today.",
  "The mouse intern says the file ran away.",
  "The cabinet laughed. We did not laugh.",

  "The owl auditor stamped it 'missing'.",
  "The pigeon suggests a fresh search from the list.",
  "We checked the shortcut. It led nowhere.",
  "The record is absent but politely so.",
  "The pigeon peered behind the curtain. Nothing.",
  "The turtle from HR recommends a quiet reset.",
  "The raccoon accountant says the trail is cold.",
  "The mouse intern checked the name twice. No match.",
  "The file took a break without notice.",
  "The pigeon suspects a very small typo.",

  "We checked the registry again. It did not change.",
  "The record is missing. The floor remains calm.",
  "The Clipboard says 'nope' in neat handwriting.",
  "The pigeon recommends going home and trying again.",
  "We asked the index for help. It blinked.",
  "The record has wandered off to somewhere else.",
  "The pigeon filed it under 'lost for now'.",
  "The mouse intern says the trail went cold.",
  "The cabinet was empty and unbothered.",
  "The record was expected. The record did not arrive.",

  "The pigeon checked the list. The list shrugged.",
  "The owl auditor took a long pause and moved on.",
  "The turtle from HR says this is temporary.",
  "The pigeon checked the Clipboard and found only quiet.",
  "The record is missing. The pigeon is still organized.",
];

// Password reset quips rotate on the reset screen; keep tone reassuring and gentle.
export const passwordreset = [
  "The pigeon updated the locks. You're welcome to do the same.",
  "The pigeon says new credentials, new possibilities.",
  "The mouse intern wrote down the old password and then ate the note. Secure.",
  "The pigeon believes fresh starts are underrated.",
  "The raccoon accountant says the old password has been officially retired.",
  "The pigeon changed the locks and held the door politely.",
  "The turtle from HR reminds you: take your time, no rush.",
  "The pigeon says this is between you and the clipboard.",
  "The seagull tried to guess the old password. The pigeon was not impressed.",
  "The pigeon marked this moment 'quietly significant'.",

  "The mouse intern salutes your security consciousness.",
  "The pigeon prepared a clean slate and a dignified nod.",
  "The raccoon accountant approved the change without blinking.",
  "The pigeon says the old password served well and may now rest.",
  "The turtle from HR suggests a password you'll remember fondly.",
  "The pigeon checks the door twice. Then hands you the key.",
  "The mouse intern labeled this 'Operation Fresh Start'.",
  "The pigeon says this is a routine procedure, nothing dramatic.",
  "The seagull is watching. The pigeon closed the blinds.",
  "The pigeon adjusted the security settings and smiled once.",

  "The clipboard is ready for the new password. It has no opinions.",
  "The pigeon recommends something strong but memorable.",
  "The raccoon accountant says the old password has been zeroed out.",
  "The mouse intern made a tiny flag that says 'new beginning'.",
  "The pigeon says you're doing the right thing.",
  "The turtle from HR approves of this security moment.",
  "The pigeon cleared the hallway for privacy.",
  "The clipboard has been turned to a fresh page. Literally.",
  "The seagull suggested 'password123'. The pigeon declined.",
  "The pigeon says the important part is that you're here.",

  "The raccoon accountant says this counts as responsible behavior.",
  "The pigeon opened a new chapter in the ledger. Figuratively.",
  "The mouse intern is learning about security. This is a teachable moment.",
  "The pigeon says trust the process, it's a good one.",
  "The turtle from HR says patience is also a form of security.",
  "The pigeon double-checked the door. It's secure.",
  "The clipboard says 'proceed'. The pigeon agrees.",
  "The seagull tried the old handle. Locked. Good.",
  "The pigeon is quietly confident this will go well.",
  "The raccoon accountant notes this is standard procedure.",

  "The pigeon placed a tiny 'under construction' sign and removed it.",
  "The mouse intern thinks you're very responsible.",
  "The pigeon says the clipboard is ready when you are.",
  "The turtle from HR says there's no wrong pace for this.",
  "The pigeon confirmed the old password is no longer welcome.",
  "The clipboard has been sanitized for your convenience.",
  "The raccoon accountant adjusted the security budget slightly upward.",
  "The pigeon stands by respectfully. This is your moment.",
  "The seagull is distracted by something shiny. Excellent timing.",
  "The pigeon says new password, new chapter, same good taste.",

  "The mouse intern labeled the old password 'retired with honors'.",
  "The pigeon says security is a journey, not a destination.",
  "The turtle from HR nodded slowly. That means approval.",
  "The pigeon has cleared the area of eavesdroppers. Especially the seagull.",
  "The clipboard is blank and ready. No pressure.",
  "The raccoon accountant says the numbers support this decision.",
  "The pigeon adjusted its tiny badge and stepped aside respectfully.",
  "The mouse intern is taking notes on good security habits.",
  "The pigeon believes this will only take a moment.",
  "The seagull was told to wait outside. The pigeon was firm.",

  "The pigeon says this is the responsible thing and it appreciates it.",
  "The clipboard has been notarized. Unnecessarily, but thoroughly.",
  "The raccoon accountant says the old password is off the books.",
  "The turtle from HR says this is exactly the right step.",
  "The pigeon checked the perimeter. All clear.",
  "The mouse intern drew a tiny lock icon. It's very cute.",
  "The pigeon says the system is ready and so are you.",
  "The seagull claims it forgot its own password too. Unverifiable.",
  "The pigeon marked this as 'in progress' and waited politely.",
  "The clipboard has no memory of the old password. Neither should you.",

  "The raccoon accountant filed the old password under 'history'.",
  "The pigeon opened a fresh form and handed you the pen.",
  "The mouse intern says this is the bravest thing they've witnessed today.",
  "The turtle from HR reminds you that everyone resets sometimes.",
  "The pigeon says the door is ready for a new key.",
  "The seagull is loitering. The pigeon loitered back. The seagull left.",
  "The clipboard has been dusted off for the occasion.",
  "The pigeon says you're in good hands. Its tiny, professional hands.",
  "The raccoon accountant confirmed this is the right form.",
  "The pigeon believes in you and your future password.",
];

// Login quips rotate on the login screen to add personality.
export const login = [
  "The pigeon checks the clipboard. You’re on the list.",
  "The pigeon adjusts the badge reader and nods approvingly.",
  "The mouse intern holds the door and whispers ‘they’re cool’.",
  "The pigeon says credentials first, then vibes.",
  "The seagull tried to sneak in earlier. The pigeon remains undefeated.",
  "The turtle from HR reminds everyone there’s no rush.",
  "The pigeon cleared this login space just for you.",
  "The clipboard says ‘expected’. The pigeon agrees.",
  "The mouse intern is watching respectfully.",
  "The pigeon believes this will go smoothly.",

  "The pigeon double-checks the door. It opens politely.",
  "The raccoon accountant confirms access is approved.",
  "The pigeon offers a calm, professional nod.",
  "The mouse intern practiced this moment.",
  "The pigeon says this is the right entrance.",
  "The seagull is not allowed past this point.",
  "The turtle from HR suggests a steady pace.",
  "The pigeon flips the sign from ‘closed’ to ‘welcoming’.",
  "The clipboard is ready when you are.",
  "The pigeon has prepared a dignified silence.",

  "The pigeon recognizes you immediately.",
  "The mouse intern beams. This is exciting.",
  "The pigeon checks once. Then once more. All good.",
  "The raccoon accountant approves the numbers involved.",
  "The pigeon says you’re right on time.",
  "The turtle from HR nods slowly but meaningfully.",
  "The pigeon straightens the entry sign.",
  "The mouse intern whispers encouragement professionally.",
  "The pigeon considers this a routine success.",
  "The clipboard has no objections today.",

  "The pigeon gently gestures toward the login fields.",
  "The mouse intern labels this ‘a big moment’.",
  "The pigeon confirms this is the correct doorway.",
  "The seagull is watching from outside. Jealous.",
  "The turtle from HR says everyone belongs here calmly.",
  "The pigeon checks credentials with care and trust.",
  "The raccoon accountant notes this as ‘authorized’.",
  "The pigeon says access looks good from here.",
  "The mouse intern adjusts their tiny badge.",
  "The pigeon opens the door metaphorically.",

  "The pigeon is pleased to see you return.",
  "The clipboard says this is familiar territory.",
  "The seagull would like you to think that she forgot her keys on purpose",
  "The pigeon verifies calmly, without judgment.",
  "The turtle from HR approves the tone of this moment.",
  "The pigeon marks this as a safe entry point.",
  "The raccoon accountant says everything checks out.",
  "The pigeon believes you belong on the other side of this door.",
  "The mouse intern learned about logins yesterday.",
  "The pigeon is ready when you are.",
];
