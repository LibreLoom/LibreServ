...
func (cm *CaddyManager) AddRoute(ctx context.Context, subdomain, domain, backend, appID string) (*Route, error) {
	...
	if err := cm.reloadCaddy(); err != nil {
		...
		slog.Warn("Route added but Caddy reload failed (kept route)", "component", "caddy", "route", route.FullDomain(), "err", err)
	}

	slog.Info("Route added", "component", "caddy", "route", route.FullDomain(), "backend", route.Backend)
	return route, nil
}
...
func (cm *CaddyManager) AddDomainRoute(ctx context.Context, domain, backend, comment string) (*Route, error) {
	...
	if err := cm.reloadCaddy(); err != nil {
		...
		slog.Warn("Route updated but Caddy reload failed (kept route)", "component", "caddy", "route", route.FullDomain(), "err", err)
	}
	return route, nil
}
...
func (cm *CaddyManager) RemoveRoute(ctx context.Context, routeID string) error {
	...
	if err := cm.deleteRoute(ctx, routeID); err != nil {
		slog.Warn("Failed to delete route from database", "component", "caddy", "err", err)
	}

	slog.Info("Route removed", "component", "caddy", "route", route.FullDomain())
	return nil
}
...
func (cm *CaddyManager) UpdateRoute(ctx context.Context, routeID string, backend string, enabled bool) (*Route, error) {
	...
	if err := cm.updateRouteInDB(ctx, route); err != nil {
		slog.Warn("Failed to update route in database", "component", "caddy", "err", err)
	}
...
func (cm *CaddyManager) reloadCaddy() error {
	...
	if cm.config.AdminAPI != "" {
		...
		if lastErr != nil {
			slog.Warn("Caddy admin reload failed after retries; attempting CLI reload", "component", "caddy", "err", lastErr)
		}
	}
