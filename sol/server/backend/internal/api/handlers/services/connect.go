...
func (h *ConnectHandler) Status(w http.ResponseWriter, r *http.Request) {
	if h.checker == nil {
		JSON(w, http.StatusOK, connect.ConnectStatus{
			Connected: false,
...
func (h *ConnectHandler) Activate(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		JSONError(w, http.StatusBadRequest, "Could not read the activation Connect key. Please check and try again.")
		return
	}
	if req.ConnectKey == "" {
		JSONError(w, http.StatusBadRequest, "Please enter your Connect key.")
		return
	}
...
func (h *ConnectHandler) UpdateServices(w http.ResponseWriter, r *http.Request) {
	...
	default:
		JSONError(w, http.StatusBadRequest, "Invalid service state. Use 'connected', 'byo', or 'disabled'.")
		return
	}

	_, exists := connect.DefaultServiceStates()[svcID]
	if !exists {
		JSONError(w, http.StatusBadRequest, "Unknown service.")
		return
	}
	...
	if svcID == connect.ServiceSupport {
		JSONError(w, http.StatusBadRequest, "Human Support comes with your Connect plan and can't be turned off here.")
		return
	}
	...
	if svcState == connect.ServiceConnected {
		...
		if provErr != nil {
			...
			JSONError(w, http.StatusBadGateway, "Could not enable this service through Connect. Please try again.")
			return
		}
		...
		if err := h.applyCredentials(r.Context(), svcID, creds); err != nil {
			...
			JSONError(w, http.StatusBadGateway, fmt.Sprintf("We couldn't apply the Connect settings for %s to your server. %v", svcID, err))
			return
		}
	}
	...
	if h.settingsService != nil {
		if err := h.settingsService.UpdateSettings(r.Context(), map[string]interface{}{
			...
			JSONError(w, http.StatusInternalServerError, "Could not save the service state. Please try again.")
			return
		}
	} else {
		cfg := config.Get()
...
func (h *ConnectHandler) applyCredentials(ctx context.Context, svcID connect.ServiceID, creds *connect.ProvisionedCredentials) error {
	...
	case connect.ServiceDomain:
		...
		if err := shared.ValidateDomain(domain); err != nil {
