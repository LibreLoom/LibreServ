...
func TestIsSecureRequest(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := shared.IsSecureRequest(req); got != tc.want {
				t.Fatalf("shared.IsSecureRequest() = %v, want %v", got, tc.want)
			}
		})
	}
}
...
func TestClearAuthCookiesMatchesSecureFlag(t *testing.T) {
	...
	for _, c := range rr.Result().Cookies() {
		s := c.Secure
		switch c.Name {
		case shared.AccessCookieName:
			accessSecure = &s
		case shared.RefreshCookieName:
			refreshSecure = &s
		}
	}
