...
func clearAuthCookies(w http.ResponseWriter, r *http.Request) {
	...
	secure := shared.IsSecureRequest(r)
	http.SetCookie(w, &http.Cookie{
		Name:     shared.AccessCookieName,
		Value:    "",
		Path:     "/",
...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	shared.RecordSecurityEvent(r.Context(), h.securityService, &event)
	h.securityService.ClearFailedAttempts(clientIP, response.User.Username)

	// Set access + refresh tokens as HTTP-only cookies.
	if err := setSessionCookies(w, h.authService, response.Tokens, shared.IsSecureRequest(r)); err != nil {
...
// transport (shared.IsSecureRequest) so sessions work over plain http:// (setup,
...
func setSessionCookies(w http.ResponseWriter, authService *auth.Service, tokens *auth.TokenPair, secure bool) error {
	...
	http.SetCookie(w, &http.Cookie{
		Name: shared.AccessCookieName, Value: tokens.AccessToken, Path: "/",
		Expires:  time.Unix(tokens.ExpiresAt, 0),
		HttpOnly: true, SameSite: http.SameSiteStrictMode, Secure: secure,
	})
	http.SetCookie(w, &http.Cookie{
		Name: shared.RefreshCookieName, Value: tokens.RefreshToken, Path: "/",
...
func (h *AuthHandler) RefreshToken(w http.ResponseWriter, r *http.Request) {
	...
	if req.RefreshToken == "" {
		if cookie, err := r.Cookie(shared.RefreshCookieName); err == nil {
...
func (h *AuthHandler) ConfirmPasswordReset(w http.ResponseWriter, r *http.Request) {
	...
	if shared.RejectBreachedPassword(w, req.NewPassword) {
		return
	}
