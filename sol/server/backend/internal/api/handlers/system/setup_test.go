...
func newTestSetupHandler(t *testing.T) testSetupDeps {
	...
	shared.SetHIBPRangeURL(hibpStub.URL + "/range/")
	dir := t.TempDir()
	...
	return testSetupDeps{
		handler:  NewSetupHandler(svc, setupSvc, (*podman.Client)(nil), nil, nil, nil, nil),
		authSvc:  svc,
		setupSvc: setupSvc,
...
func TestPreflightResolvesRelativeDiskSpacePathFromConfigLocation(t *testing.T) {
	...
	handler := NewSetupHandler(authSvc, setupSvc, (*podman.Client)(nil), nil, nil, nil, nil)
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/api/v1/setup/preflight", nil)
...
func TestFrontendStepContractKeptInSync(t *testing.T) {
	...
	// Source: STEP map in sol/server/frontend/src/pages/SetupPage.jsx.
...
func TestCheckBreachedPassword(t *testing.T) {
	...
	defer stub.Close()
	shared.SetHIBPRangeURL(stub.URL + "/range/")
	defer shared.SetHIBPRangeURL(shared.DefaultHIBPRangeURL)

	// Breached password -> true
	breached, err := shared.CheckBreachedPassword("password123")
	if err != nil {
	...
	breached, err = shared.CheckBreachedPassword("Tr0ub4dor&3-Good!")
	if err != nil {
...
func TestCheckBreachedPasswordFailsOpen(t *testing.T) {
	shared.SetHIBPRangeURL("http://127.0.0.1:1/range/") // nothing listens here
	defer shared.SetHIBPRangeURL(shared.DefaultHIBPRangeURL)

	breached, err := shared.CheckBreachedPassword("whatever123")
	if err == nil {
