...
func TestBackupRepositoryHandlers(t *testing.T) {
	...
	// These assertions depend on whether a restic binary is available in the
	// environment: with restic, repo creation succeeds; without it, the
	// handler returns 412 Precondition Failed.
	if service.UseRestic() {
	...
	testRepoWant := http.StatusPreconditionFailed
	if service.UseRestic() {
		testRepoWant = http.StatusOK // empty config fails the probe but returns 200 with success:false
	}
	callBackupHandler(t, handler.TestRepository, httptest.NewRequest(http.MethodPost, "/repositories/test", strings.NewReader(`{}`)), testRepoWant)
	rec = callBackupHandler(t, handler.GetBackupCapabilities, httptest.NewRequest(http.MethodGet, "/capabilities", nil), http.StatusOK)
	wantAvail := `"restic_available":false`
	if service.UseRestic() {
		wantAvail = `"restic_available":true`
	}
	if !strings.Contains(rec.Body.String(), wantAvail) {
